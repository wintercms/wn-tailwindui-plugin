...
>
> ✅ **G23 (Builder DataTable dropdowns) — FIXED & verified. [CORE, not Builder-owned]** Turned out to be core `table.css`, not Builder CSS: the Table-widget cell dropdown (`.table-control-dropdown-list`) is painted white with `#555` text on non-focused items, and an active dropdown cell's `.content-container` goes white while editing. Fixed both in `darkmode.css` control-table section (dark list bg/border + light item text + dark active-cell container). Verified on the Builder migration/column-type dropdown (scratch plugin `plugins/scratch/darkaudit`): list `#161b22`, items `#c9d1d9`, active cell `#010409`. **The whole Builder Database/table editor is now fully dark via core fixes** (0 remaining light elements). G24 (model form-builder) + G25 (backend-menu editor) still need capture — the Builder's own CSS (`builder.less` + imports) has extensive hardcoded light values (`.auxiliary-tabs{background:white}`, `menus.less`/`buildingarea.less` white bgs, `#bdc3c7`/`#ecf0f1`/`#d9d9d9` borders); those are plugin-owned and blocked on: (a) reliable Builder plugin-selection via automation (the active-plugin JS state resets on reload and resists scripted clicks), and (b) `winter:util compile less` aborting on `.clearfix` (the mixin IS defined via boot.less→global.less; the util command mis-resolves sub-imports out of context — needs infra fix or hand-sync of `builder.css`).
>
