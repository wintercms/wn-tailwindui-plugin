...
>
> ✅ **G20 (theme-editor component list) — FIXED & verified.** The Components sidebar (`.control-filelist.component-list`) divided its group rows with a light `#e6ebed` top border. Darkened `.control-filelist.component-list ul li.group` border-top to `var(--drk-border-color)`. Verified `backend/cms` Components (`rgb(48,54,61)`, list reads cleanly). [core CMS → darkmode.css]
>
> ✅ **G19 (CMS/Builder master-area tab wings) — FIXED & verified.** After the G1 fix darkened the active `.master-area` tab *title* to `#21262d`, its skewed wings (`::before`/`::after`, drawn in `fancy-layout.css`) stayed hardcoded white `#fafafa`, so the active CMS/Builder tab read as a dark pill flanked by two white slivers. Added a dark override next to the source (unlayered, so Tailwind won't purge it): `html[data-color-scheme=dark] …master-area…span.title::before/::after { background:#21262d }`. Verified on `backend/cms` 404-page editor (Settings tab: title + wings all `#21262d`). NB: a separate minor codeeditor "Markup" section tab still shows white in the CMS editor's code header — transient/hard to pin; left for a follow-up.
>
> ✅ **G18 + G19 (Winter.Pages list + menu-editor tabs) — VERIFIED CLEAN.** Re-checked `backend/winter/pages`: the Pages tree and Menus list render dark/clean; the menu editor's tab ("Menu items") uses the same `.control-tabs.primary-tabs.master-area`, so the **G19 wing fix covers it too** (active title + wings `#21262d`, 0 light elements in the editor). The delete button is the standard danger red (see G17). No Winter.Pages-specific changes needed — all handled by the core darkmode.css + fancy-layout.css fixes.
>
> ⏸️ **G17 (CMS template delete button) — NOT A DARK-MODE BUG.** The delete button is `.btn-danger` (`bg-red-600 #dc2626` in dark, `bg-red-500` in light) — the theme's standard danger button, identical in both modes and consistent with every other Delete button; the `.empty` class isn't styled by the skin, so it renders solid rather than ghost. That's a cross-theme design choice (a solid-red icon button among ghost toolbar buttons), not dark-specific. No change made.
>
> ✅ **G23 (Builder DataTable dropdowns) — FIXED & verified. [CORE, not Builder-owned]** Turned out to be core `table.css`, not Builder CSS: the Table-widget cell dropdown (`.table-control-dropdown-list`) is painted white with `#555` text on non-focused items, and an active dropdown cell's `.content-container` goes white while editing. Fixed both in `darkmode.css` control-table section (dark list bg/border + light item text + dark active-cell container). Verified on the Builder migration/column-type dropdown (scratch plugin `plugins/scratch/darkaudit`): list `#161b22`, items `#c9d1d9`, active cell `#010409`. **The whole Builder Database/table editor is now fully dark via core fixes** (0 remaining light elements). **G24/G25 — ADDRESSED. [plugin-owned: Winter.Builder, branch `wip/darkmode-editors`, commit `23f5b5e`, pushed]** Added a standalone `assets/css/darkmode.css` (loaded after `builder.css` in `controllers/Index.php`) rather than editing the `.less` bundle — because `winter:util compile less` aborts on a `.clearfix undefined` resolution bug in `buildingarea.less` (the mixin *is* defined via boot.less→global.less; the util command mis-resolves sub-imports out of context, so `builder.css` can't be regenerated cleanly). The dark-scoped file covers the building-area/control-blueprints (G24), backend-menu editor (G25), auxiliary tabs and the inspector border, all from the `.less` source audit. **G25 verified end-to-end** (menu-editor canvas `#0d1117`, add-box border `#30363d`, item cards dark). **G24 rules written from the `.less` source** but not visually confirmed — reaching the form-builder needs a Builder model (table→model→form), which proved too fiddly to create via automation this session; the rules are dark-scoped so light mode is unaffected. **Builder plugin-list capture solved:** the list defaults to a *"my plugins"* filter (`PluginList.php` — matches the Builder author-namespace setting); the funnel toggle switches to *"all plugins"* which populates it. Non-Builder plugins (Blog etc.) can't have their models/menus edited ("Settings of this plugin cannot be edited with Builder") — need a Builder-created plugin.
>
